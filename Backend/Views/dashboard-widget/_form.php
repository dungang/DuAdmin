...
<div class="dashboard-widget-form">
    ...
    <div class="row">
        ...
        <?= '<div class="col-xs-6">' . $form->field($model, 'status')->radioList(DictData::getDataLabels("online_status")) . '</div>' ?>

        <?= '<div class="col-xs-6">' . $form->field($model, 'sort')->textInput() . '</div>' ?>

        <?= '<div class="col-xs-6">' . $form->field($model, 'type')->dropDownList(DictData::getDataLabels("dashboard_widget_type")) . '</div>' ?>
