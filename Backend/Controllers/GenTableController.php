...
use DuAdmin\Models\GenTableSearch;
use DuAdmin\Core\BackendController;

/**
 * GenTable 模型的控制器
 * GenTableController 实现了常规的增删查改等行为
 */
class GenTableController extends BackendController
{
    /**
     * 请求action 方法设置
     * @var array
    ...
    {
        $searchModel = new GenTableSearch();
    ...
    {
    	if( is_array($id) ) {
    		$modelList = GenTable::findAll(['id' => $id]);
    		if( $modelList ) {
    ...
    /**
     * 根据模型的主键Id查询 GenTable 模型.
     * 如果模型没有找到,  404 HTTP 异常将会抛出.
     * @param integer $id
     * @return GenTable the loaded model
     * @throws NotFoundHttpException 如果模型没查询到
     */
    protected function findModel($id)
    {

        if (($model = GenTable::findOne(['id' => $id])) !== null) {
            return $model;
        }
