...
use DuAdmin\Models\PrettyUrlSearch;
use DuAdmin\Core\BackendController;

/**
 * PrettyUrl 模型的控制器
 * PrettyUrlController 实现了常规的增删查改等行为
 */
class PrettyUrlController extends BackendController
{
    ...
    {
        $searchModel = new PrettyUrlSearch();
    ...
    {
    	if( is_array($id) ) {
    		$modelList = PrettyUrl::findAll(['id' => $id]);
    		if( $modelList ) {
    ...
    /**
     * 根据模型的主键Id查询 PrettyUrl 模型.
     * 如果模型没有找到,  404 HTTP 异常将会抛出.
     * @param integer $id
     * @return PrettyUrl the loaded model
     * @throws NotFoundHttpException 如果模型没查询到
     */
    protected function findModel($id)
    {

        if (($model = PrettyUrl::findOne(['id' => $id])) !== null) {
            return $model;
        }
