...
use DuAdmin\Models\DictDataSearch;
use DuAdmin\Core\BackendController;

/**
 * DictData 模型的控制器
 * DictDataController 实现了常规的增删查改等行为
 */
class DictDataController extends BackendController
{
    /**
     * 请求action 方法设置
     * @var array
    ...
    {
        $searchModel = new DictDataSearch();
    ...
    {
    	if( is_array($id) ) {
    		$modelList = DictData::findAll(['id' => $id]);
    		if( $modelList ) {
    ...
    /**
     * 根据模型的主键Id查询 DictData 模型.
     * 如果模型没有找到,  404 HTTP 异常将会抛出.
     * @param integer $id
     * @return DictData the loaded model
     * @throws NotFoundHttpException 如果模型没查询到
     */
    protected function findModel($id)
    {

        if (($model = DictData::findOne(['id' => $id])) !== null) {
            return $model;
        }
