...
<?php  \yii\bootstrap\Modal::begin([
        'id' => 'source-message-search-modal',
        'header' => '高级搜索',
...
<div class="source-message-search">
    ...
    <?php $form = ActiveForm::begin([
        ...
        'options' => [
                'data-pjax' => 1,
                'onsubmit' => "\$('#source-message-search-modal').modal('hide')"
        ],
    ]); ?>
<div class="row">
    <?= '<div class="col-xs-6">' . $form->field($model, 'id') . '</div>' ?>

    <?= '<div class="col-xs-6">' . $form->field($model, 'category') . '</div>' ?>

    <?= '<div class="col-xs-6">' . $form->field($model, 'message')->widget('DuAdmin\Widgets\DefaultEditor') . '</div>' ?>
