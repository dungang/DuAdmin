...
if (isset($category)) {
    if ($category->pid == 0) {
        ...
        $this->params['breadcrumbs'][] = [
            'label' => $rootCategory,
            'url' => ['/cms/post/index', 'slug' => $parent_category->slug]
        ];
    ...
    <div class="row">
                ...
                <div class="panel-body">
                    <?= Nav::widget([
                        ...
                        'items' => array_map(function ($item) use ($category) {
                            ...
                            if ($category && $category->slug == $item['slug']) {
                                $active = true;
                            } else {
                                $active = false;
                            }
                            return [
                                'label' => $item['name'],
                                'active' => $active,
                                'url' => ['/cms/post/index', 'slug' => $item['slug']]
                            ];
                        }, $subCategories)
                    ]) ?>
                </div>
        ...
        <div class="col-md-9">
            ...
            echo ListView::widget([
                'options' => ['class' => 'list-product fix-col-height',],
                'layout' => "{items}\n{pager}",
                'dataProvider' => $dataProvider,
                'itemView' => function ($model, $key, $index, $widget) {
                    return $this->render('item-product', ['model' => $model]);
                }
            ]);
            ?>
        </div>
