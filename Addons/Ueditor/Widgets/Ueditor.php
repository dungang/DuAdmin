...
{
    public $serverUrl = ['/ueditor/upload/index'];
    ...
    public function init()
    {
        ...
        $this->clientOptions['toolbars'] = [array_merge($baseBars, $this->getBars($this->mode))];
    ...
    public function getBars($mode = self::MODE_DEFAULT)
    {
        if ($mode == self::MODE_RICH) {
            return ['|', 'imagenone', 'imageleft', 'imageright', 'imagecenter', '|', 'insertimage', 'insertvideo','|','map'];
        } else {
            return [];
        }
    }
