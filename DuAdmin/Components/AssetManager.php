...
class AssetManager extends Component {
   ...
   * @param bool $publish
   *          whether to publish the asset files in the asset bundle before it is returned.
   ...
   * @param bool|null $appendTimestamp
   ...
   *
   * @param array|AssetConverterInterface $value
   *          the asset converter. This can be either
  ...
  protected function publishDirectory( $src, $options ) {
    ...
    $this->checkBasePathPermission();
    $dir = null;
    if ( isset( $options['publishDir'] ) && ! empty( $options['publishDir'] ) ) {
